use crate::{nn::Individual, random::normal_sample};
...
const MAX_BALL_Y_VEL: f32 = MAX_PADDLE_VEL * 1.75 * FRAME_RATE;
...
impl Game {
    ...
    pub fn run_until(
        ...
        loop {
            if ticks >= max_ticks {
                // println!("Max ticks reached by: {:?}", left_controller.weights());
        ...
        if self.ball_pos_x() <= left_x_bound {
                ...
                && self.ball_pos_y() <= self.left_pos() + PADDLE_LENGTH / 2.
            {
                ...
                *self.ball_vel_y_mut() += self.left_vel() * 0.25
                    + normal_sample(DEF_MEAN, DEF_STD_DEV)
                        * BALL_START_VEL
                        / MAX_PADDLE_VEL;
