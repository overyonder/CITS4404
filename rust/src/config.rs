...
pub enum ReproductionStrategy {
    /// **C++ Equivalent:** The top `sqrt(N)` individuals survive. The next generation
    /// is formed by crossing over every pair of survivors, then filling the rest of
    /// the population with mutated copies of the survivors.
    #[default]
    CppEquivalent,
    /// **Modern:** A configurable number of `elite_count` individuals are carried over.
    /// The rest of the population is generated by selecting two random elites, crossing
    /// them over, and then mutating the offspring.
    Modern,
}
