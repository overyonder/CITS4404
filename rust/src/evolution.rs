...
impl<I: Individual> Population<I> {
    ...
    fn calculate_diversity_metrics(&self) -> (f32, f32) {
        ...
        let sample_size = 100.min(crate::engines::constants::TOTAL_WEIGHTS); // Sample weights for efficiency
        let mut weight_variances = Vec::new();
        
        for weight_idx in (0..crate::engines::constants::TOTAL_WEIGHTS).step_by(crate::engines::constants::TOTAL_WEIGHTS / sample_size) {
    ...
    pub fn evolve(&mut self, tx: Option<mpsc::Sender<TrainingMessage>>) -> I {
        ...
        for gen in 0..self.config.generations {
            ...
            let total_matches_this_gen = match self.config.engine {
                ...
                // GPU engine now does full round-robin like CPU, so same match count
