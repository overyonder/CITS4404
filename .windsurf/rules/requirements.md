...
- [ ] Fitness history should use a bar graph instead of blocks. It must also show the actual fitness number so we know the relative strength as generations progress.
- [ ] The blocks for each section should use nice borders with BorderType::Rounded rather than dash characters.
- [ ] The percentage progress gauge is helpful but under it, we should have a frame with text for the number of generations done / remaining / currently processing. Under this, we should have blocks showing all of the generations/matchups to be completed (toggleable with tabs). These should be grey blocks, similar to a defragmenter UI. When the generation/matchup is complete, it should turn green. When it is being executed (e.g. currently running on the relevant engine) it should turn orange. This means that multiple blocks might be orange in concurrent mode.
- [ ] The champion genome should be shown as 7 x 31 blocks (it is 217 parameters). The colour of each block should depend on the intensity of that weight (since each is a 32 bit float, this should be able to be normalized to an RGB value).
- [ ] The log should actually log the messages from tracing in a scrolling log window. Currently it only shows "starting training".
